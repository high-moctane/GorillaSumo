...
public class AgentM1 : Agent
{
    ...
    public AgentM1(Agent.ID ID) : base(ID) {
        myID = (int)ID;
    }
...
public class AgentM3 : Agent
{
    ...
    public AgentM3(Agent.ID ID) : base(ID) {
        myID = (int)ID;
    }
    ...
    public override Action Step(State s)
    {
        if (
            (Mathf.Abs(s.position[myID].x) > 1.5f ||
            Mathf.Abs(s.position[myID].z) > 1.5f) &&
            (Mathf.Abs(s.velocity[myID].x) > 1f ||
            Mathf.Abs(s.velocity[myID].z) > 1f)
        ) {
            return PID(s);
        }
        return Chase(s);
